...
func TestSetDNS(t *testing.T) {
	...
	t.Run("valid servers writes resolv.conf with correct content", func(t *testing.T) {
		...
		executor.commands["rm -f /var/tmp/net_resolv.conf"] = ""
		executor.commands["tee /var/tmp/net_resolv.conf"] = ""
		executor.commands["mv /var/tmp/net_resolv.conf /etc/resolv.conf"] = ""
		executor.commands["chattr +i /etc/resolv.conf"] = ""
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
		...
		executor.assertCommandExecuted(t, "tee /var/tmp/net_resolv.conf")
		executor.assertCommandExecuted(t, "mv /var/tmp/net_resolv.conf /etc/resolv.conf")
		executor.assertCommandExecuted(t, "chattr +i /etc/resolv.conf")
		executor.assertInputContains(t, "tee /var/tmp/net_resolv.conf", "nameserver 8.8.8.8")
		executor.assertInputContains(t, "tee /var/tmp/net_resolv.conf", "nameserver 1.1.1.1")
	})
	...
	t.Run("invalid IP addresses are filtered out", func(t *testing.T) {
		...
		input := executor.inputsReceived["tee /var/tmp/net_resolv.conf"]
	...
	t.Run("tee failure returns error", func(t *testing.T) {
		...
		executor.commands["chattr -i /etc/resolv.conf"] = ""
		executor.commands["rm -f /var/tmp/net_resolv.conf"] = ""
		executor.errors["tee /var/tmp/net_resolv.conf"] = fmt.Errorf("permission denied")
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
	...
	t.Run("mv failure returns error", func(t *testing.T) {
		...
		executor.commands["rm -f /var/tmp/net_resolv.conf"] = ""
		executor.commands["tee /var/tmp/net_resolv.conf"] = ""
		executor.errors["mv /var/tmp/net_resolv.conf /etc/resolv.conf"] = fmt.Errorf("permission denied")
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
...
func TestWriteFile(t *testing.T) {
	t.Run("success - removes temp, writes, and moves", func(t *testing.T) {
		...
		executor.commands["rm -f /var/tmp/net_resolv.conf"] = ""
		executor.commands["tee /var/tmp/net_resolv.conf"] = ""
		executor.commands["mv /var/tmp/net_resolv.conf /etc/test.conf"] = ""
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
		...
		executor.assertCommandExecuted(t, "rm -f /var/tmp/net_resolv.conf")
		executor.assertCommandExecuted(t, "tee /var/tmp/net_resolv.conf")
		executor.assertCommandExecuted(t, "mv /var/tmp/net_resolv.conf /etc/test.conf")
		executor.assertInputContains(t, "tee /var/tmp/net_resolv.conf", "test content")
	})

	t.Run("rm temp fails - continues anyway", func(t *testing.T) {
		executor := newStrictMockExecutor()
		executor.errors["rm -f /var/tmp/net_resolv.conf"] = fmt.Errorf("file not found")
		executor.commands["tee /var/tmp/net_resolv.conf"] = ""
		executor.commands["mv /var/tmp/net_resolv.conf /etc/test.conf"] = ""
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
	...
	t.Run("mv fails - returns error", func(t *testing.T) {
		...
		executor.commands["rm -f /var/tmp/net_resolv.conf"] = ""
		executor.commands["tee /var/tmp/net_resolv.conf"] = ""
		executor.errors["mv /var/tmp/net_resolv.conf /etc/test.conf"] = fmt.Errorf("permission denied")
		logger := &mockLogger{}
		manager := &Manager{executor: executor, logger: logger}
...
func TestConnectToConfiguredNetwork(t *testing.T) {
	...
	t.Run("wired connection with DHCP", func(t *testing.T) {
		...
		executor.commands["rm -f /var/lib/dhcp/dhclient.eth0.leases /tmp/dhclient.eth0.leases"] = ""
	...
	t.Run("dhclient fails", func(t *testing.T) {
		executor := &mockSystemExecutor{
			commands: map[string]string{
				...
				"rm -f /var/lib/dhcp/dhclient.eth0.leases /tmp/dhclient.eth0.leases": "",
			},
			errors: map[string]error{
